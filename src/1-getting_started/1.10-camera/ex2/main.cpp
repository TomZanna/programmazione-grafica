...
glm::mat4 custom_lookAt(glm::vec3 cameraPosition, glm::vec3 target,
                        glm::vec3 worldUp) {
  glm::vec3 cameraDirection = glm::normalize(cameraPosition - target);
  glm::vec3 rightVector = glm::normalize(glm::cross(worldUp, cameraDirection));
  glm::vec3 cameraUp = glm::cross(cameraDirection, rightVector);
  ...
  rotation[0][0] = rightVector.x; // x vector
  rotation[1][0] = rightVector.y;
  rotation[2][0] = rightVector.z;
  rotation[0][1] = cameraUp.x; // y vector
  rotation[1][1] = cameraUp.y;
  rotation[2][1] = cameraUp.z;
  rotation[0][2] = cameraDirection.x; // z vector
...
int main() {
  ...
  while (!glfwWindowShouldClose(window)) {
    ...
    view = custom_lookAt(cameraPos, cameraPos + cameraFront, worldUp);
    ...
    // Disegna tutti i cubi
    for (unsigned int i = 0; i < 10; i++) {
      glm::mat4 model = glm::mat4(1.0f);
      model = glm::translate(model, cubePositions[i]);
      float angle =
          20.0f * i + (float)glfwGetTime() * 50.0f; // rotazione animata
      model =
